...
function pillSVG(ap,mini){
  ...
  } else if(fmt.includes('caneta')){
    body = `<rect x="14" y="34" width="112" height="22" rx="10" fill="${c1}" stroke="${st}" stroke-width="1.3"/>`+
           `<rect x="40" y="38" width="30" height="14" rx="3" fill="#F4F6F3" stroke="${st}" stroke-width=".8"/><line x1="46" y1="45" x2="64" y2="45" stroke="${st}" stroke-width=".8"/>`+
           `<rect x="126" y="38" width="16" height="14" rx="3" fill="${shade(c1,-25)}" stroke="${st}"/><circle cx="148" cy="45" r="5" fill="#DDD" stroke="#888"/>`+
           `<rect x="4" y="40" width="10" height="10" rx="2" fill="#E9E9E9" stroke="#888"/>`;
  ...
  return `<svg viewBox="0 0 ${W} ${H}" role="img" aria-label="${esc((a.formato||'')+' '+(a.cor||''))}">${defs}${body}${fmt.includes('seringa')||fmt.includes('caneta')||fmt.includes('ampola')||fmt.includes('gota')?'':grav}</svg>`;
}
...
function titulacaoSVG(e){
  ...
  for(let v=0;v<=ymax+1e-9;v+=step){ s+=`<line x1="${l}" x2="${W-r}" y1="${Y(v)}" y2="${Y(v)}" stroke="var(--line)"/><text x="${l-6}" y="${Y(v)+4}" font-size="10" text-anchor="end" fill="var(--ink-3)">${(+v.toFixed(2)).toLocaleString('pt-BR')}</text>`; }
  ...
  pts.forEach(p=>{ s+=`<circle cx="${X(p[0])}" cy="${Y(p[1])}" r="4" fill="var(--surface)" stroke="var(--accent)" stroke-width="2"/><text x="${X(p[0])+5}" y="${Y(p[1])-8}" font-size="10.5" fill="var(--ink)">${Number(p[1]).toLocaleString('pt-BR')}</text>`; });
  ...
  s+=`<text x="4" y="18" font-size="10.5" text-anchor="start" fill="var(--ink-2)">${esc(e.unidade_dose||'mg/dia')}</text>`;
  s+='</svg>';
...
function trocaSVG(p){
  ...
  s+='</svg>';
  return `<figure class="fig">${s}<figcaption>${p.legenda||'Proporção da dose-alvo ao longo do tempo. Esquema ilustrativo; ajustar à dose de partida, à resposta e aos sintomas de retirada.'}</figcaption></figure>`;
}
...
/* ---------- LAYOUT ---------- */
const NAV=[['inicio','Início'],['lista','Fármacos'],['condicoes','Condições'],['psicoeducacao','Psicoeducação'],['comparar','Comparar'],['interacoes','Interações'],['equivalencias','Equivalências'],['trocas','Trocas'],['favoritos','Favoritos'],['sobre','Sobre']];
function shell(){
  app.innerHTML = `
...
function render(){
  ...
  document.querySelectorAll('#nav a').forEach(a=>{ const on = a.dataset.k===r.v || (r.v==='condicao'&&a.dataset.k==='condicoes') || (r.v==='psico'&&a.dataset.k==='psicoeducacao') || (r.v==='ficha'&&a.dataset.k==='lista') || (r.v==='classe'&&a.dataset.k==='lista'); if(on) a.setAttribute('aria-current','page'); else a.removeAttribute('aria-current'); });
  counters();
  ...
  else if(r.v==='psico') renderPsico(r.arg);
  else if(r.v==='psicoeducacao') renderPsicoLista();
  ...
  if(r.v==='psico'||r.v==='psicoeducacao') return;
...
function renderInicio(){
  ...
  <div class="tools">
    ...
    <a class="tool" href="#trocas">${I.swap}<div><b>Guia de trocas</b><span>Antidepressivos, antipsicóticos e depósito</span></div></a>
    <a class="tool" href="#psicoeducacao">${I.pdf}<div><b>Psicoeducação</b><span>${PE_N} fichas para pacientes e famílias, prontas para imprimir</span></div></a>
  </div>
...
function renderCondicao(slug){
  ...
  const blocos=[];
  const C=info.criterios;
  if(C&&arr(C.itens).length) blocos.push(['criterios','Critérios diagnósticos',`<div class="crit"><p class="crit-f"><b>${esc(C.fonte||'')}</b>${C.cid?`<span class="muted"> · ${esc(C.cid)}</span>`:''}</p>
    <ol class="crit-l">${C.itens.map(it=>{ const lg=String(it.r||'').length>3; return `<li class="${lg?'lg':''}">${lg?`<h4 class="crit-rl">${esc(it.r)}</h4>`:`<span class="crit-r">${esc(it.r||'')}</span>`}<div><p>${md(it.t||'')}</p>${arr(it.sub).length?`<ol class="crit-s${it.sub.some(x=>/^([A-Z]\d?|\d+|[a-z])[.)]\s/.test(x))?' nonum':''}">${it.sub.map(x=>`<li>${md(x)}</li>`).join('')}</ol>`:''}</div></li>`; }).join('')}</ol>
    ${arr(C.especificadores).length?`<h3 class="cp-h">Especificadores</h3>${L(C.especificadores)}`:''}
    ${arr(C.notas).length?`<h3 class="cp-h">Notas</h3>${L(C.notas)}`:''}
    <p class="small muted">Texto parafraseado e condensado para consulta; confira o manual original para uso formal.</p></div>`]);
  const RS=info.rastreio;
  if(RS&&(arr(RS.perguntas).length||arr(RS.sinais).length)) blocos.push(['rastreio','Quando suspeitar e o que perguntar',`<div class="cols2 rast"><div class="rc-b"><h3>Perguntas úteis</h3><ul class="cl perg">${arr(RS.perguntas).map(t=>`<li>${md(t)}</li>`).join('')}</ul></div><div class="rc-b"><h3>Sinais que levantam a suspeita</h3>${L(RS.sinais)}</div></div>${arr(RS.instrumentos).length?`<h3 class="cp-h">Instrumentos</h3>${L(RS.instrumentos)}`:''}`]);
  if(arr(info.diferencial).length) blocos.push(['diferencial','Diagnóstico diferencial',L(info.diferencial)]);
  if(arr(info.discussao).length) blocos.push(['discussao','Discussão clínica',`<div class="cdisc">${info.discussao.map(d=>`<article><h3>${esc(d.titulo||'')}</h3>${String(d.texto||'').split(/\n\n+/).map(t=>`<p>${md(t)}</p>`).join('')}</article>`).join('')}</div>`]);
  if(arr(info.fases).length) blocos.push(['fases','Fases do tratamento',`<ol class="cfases" style="--n:${info.fases.length}">${info.fases.map((f,i)=>`<li style="--i:${i}"><span class="cf-n">${esc(f.fase||'')}</span><span class="cf-d">${esc(f.duracao||'')}</span><p>${md(f.objetivo||'')}</p></li>`).join('')}</ol>`]);
  if(arr(info.algoritmo).length) blocos.push(['algoritmo','Algoritmo de tratamento',`<ol class="calg">${info.algoritmo.map((a,i)=>`<li><div class="ca-box"><span class="ca-n">${i+1}</span><div><b>${esc(a.passo||'')}</b><p>${md(a.acao||'')}</p></div></div>${a.se_falha&&i<info.algoritmo.length-1?`<div class="ca-seta"><span>${md(a.se_falha)}</span></div>`:a.se_falha?`<div class="ca-fim">${md(a.se_falha)}</div>`:''}</li>`).join('')}</ol>`]);
  ...
  if(arr(info.manejo_diretrizes).length) blocos.push(['manejo','Manejo segundo as diretrizes',`<div class="mdir">${info.manejo_diretrizes.map(g=>`<div class="mdir-c"><h3>${esc(g.fonte||'')}</h3>${arr(g.linhas).length?`<div class="scroll-x"><table class="itable mlin"><tbody>${g.linhas.map(x=>`<tr><th>${esc(x.linha||'')}</th><td>${md(x.texto||'')}</td></tr>`).join('')}</tbody></table></div>`:''}${L(g.pontos)}</div>`).join('')}</div>`]);
  ...
  if(arr(PE[g]).length) blocos.push(['psico','Psicoeducação ('+PE[g].length+')',peSecao(g)]);
  ...
  M.querySelectorAll('[data-ir]').forEach(a=>a.onclick=e=>{ e.preventDefault(); irPara(document.getElementById(a.dataset.ir), M.querySelector('.ctoc')); });
  document.title=g+' · Guia de Psicofármacos';
}

/* ---------- PSICOEDUCAÇÃO (fichas para pacientes e famílias) ---------- */
const PE = D.psicoeducacao || {};
const PE_N = Object.values(PE).reduce((a,l)=>a+arr(l).length,0);
/* figuras: SVG simples, legíveis em papel e em tela */
const PE_FIG = {
 'janela-sono': ()=>{ const X=h=>30+(h-18)*33; let s=`<svg viewBox="0 0 520 170" role="img" aria-label="Janela de sono">`;
   [18,20,22,24,26,28,30,32].forEach(h=>{ s+=`<line x1="${X(h)}" y1="30" x2="${X(h)}" y2="140" stroke="#cfd6d2" stroke-dasharray="2 3"/><text x="${X(h)}" y="158" font-size="12" text-anchor="middle" fill="#55605a">${(h%24)}h</text>`; });
   s+=`<text x="20" y="22" font-size="13" font-weight="700" fill="#1d2521">Antes: muito tempo na cama, sono picado</text>`;
   s+=`<rect x="${X(21.5)}" y="34" width="${X(32)-X(21.5)}" height="30" rx="6" fill="#e7ecea" stroke="#8a9892"/>`;
   [[22.5,24],[25,26.5],[28,30]].forEach(([a,b])=>{ s+=`<rect x="${X(a)}" y="38" width="${X(b)-X(a)}" height="22" rx="4" fill="#6b8f86"/>`; });
   s+=`<text x="20" y="92" font-size="13" font-weight="700" fill="#1d2521">Depois: janela menor, sono mais contínuo</text>`;
   s+=`<rect x="${X(24)}" y="102" width="${X(30.5)-X(24)}" height="30" rx="6" fill="#e7ecea" stroke="#035D51"/><rect x="${X(24.3)}" y="106" width="${X(30.3)-X(24.3)}" height="22" rx="4" fill="#035D51"/>`;
   s+=`<text x="${(X(24)+X(30.5))/2}" y="121" font-size="11" text-anchor="middle" fill="#fff" font-weight="700">horário fixo para deitar e levantar</text></svg>`; return s; },
 'ciclo-ansiedade': ()=>cicloSVG(['Situação ou pensamento','Medo e sensações no corpo','Evitar ou fugir','Alívio rápido','Medo cresce na próxima vez'],'O ciclo da ansiedade'),
 'ciclo-toc': ()=>cicloSVG(['Pensamento intrusivo (obsessão)','Angústia','Ritual (compulsão)','Alívio passageiro','Obsessão volta mais forte'],'O ciclo do TOC'),
 'ciclo-compulsao': ()=>cicloSVG(['Dieta rígida ou pular refeições','Fome e vontade intensa','Episódio de compulsão','Culpa e vergonha','Compensar ou restringir de novo'],'O ciclo da compulsão'),
 'ciclo-panico': ()=>cicloSVG(['Sensação no corpo (coração acelera)','Interpretação catastrófica','Mais medo','Mais sensações','Ataque de pânico'],'O ciclo do pânico'),
 'humor-bipolar': ()=>{ let s=`<svg viewBox="0 0 520 190" role="img" aria-label="Oscilação do humor">`;
   s+=`<rect x="40" y="20" width="470" height="45" fill="#fbe9d9"/><rect x="40" y="65" width="470" height="50" fill="#e3f0ea"/><rect x="40" y="115" width="470" height="45" fill="#e2e9f5"/>`;
   s+=`<text x="46" y="36" font-size="12" fill="#8a4b12" font-weight="700">Mania ou hipomania</text><text x="46" y="82" font-size="12" fill="#035D51" font-weight="700">Humor estável</text><text x="46" y="154" font-size="12" fill="#1f4e8c" font-weight="700">Depressão</text>`;
   s+=`<path d="M40 90 C90 90 100 95 130 92 S170 30 200 35 S240 90 260 92 S300 150 330 145 S370 90 400 90 S460 88 510 90" fill="none" stroke="#1d2521" stroke-width="3"/>`;
   s+=`<text x="275" y="182" font-size="12" text-anchor="middle" fill="#55605a">tempo (meses a anos): o tratamento busca deixar a linha na faixa verde</text></svg>`; return s; },
 'curva-remedio': ()=>{ const X=w=>50+w*55, Y=v=>150-v*110; let s=`<svg viewBox="0 0 520 190" role="img" aria-label="Efeito do remédio ao longo das semanas">`;
   s+=`<line x1="50" y1="150" x2="500" y2="150" stroke="#8a9892"/><line x1="50" y1="30" x2="50" y2="150" stroke="#8a9892"/>`;
   for(let w=0;w<=8;w++) s+=`<text x="${X(w)}" y="168" font-size="12" text-anchor="middle" fill="#55605a">${w}</text>`;
   s+=`<text x="275" y="186" font-size="12" text-anchor="middle" fill="#55605a">semanas de uso</text>`;
   s+=`<path d="M${X(0)} ${Y(0)} C${X(.5)} ${Y(.55)} ${X(1)} ${Y(.5)} ${X(2)} ${Y(.25)} S${X(4)} ${Y(.08)} ${X(8)} ${Y(.05)}" fill="none" stroke="#c0562f" stroke-width="3"/>`;
   s+=`<path d="M${X(0)} ${Y(0)} C${X(1.5)} ${Y(.05)} ${X(2)} ${Y(.2)} ${X(3.5)} ${Y(.55)} S${X(6)} ${Y(.9)} ${X(8)} ${Y(.95)}" fill="none" stroke="#035D51" stroke-width="3"/>`;
   s+=`<text x="${X(1.1)}" y="${Y(.62)}" font-size="12" fill="#c0562f" font-weight="700">efeitos colaterais: mais no início</text><text x="${X(4.2)}" y="${Y(.98)}" font-size="12" fill="#035D51" font-weight="700">melhora: aparece aos poucos</text></svg>`; return s; },
 'onda-fissura': ()=>{ const X=m=>50+m*14, Y=v=>150-v*110; let s=`<svg viewBox="0 0 520 190" role="img" aria-label="A fissura passa como uma onda">`;
   s+=`<line x1="50" y1="150" x2="500" y2="150" stroke="#8a9892"/>`;[0,10,20,30].forEach(m=>{ s+=`<text x="${X(m)}" y="168" font-size="12" text-anchor="middle" fill="#55605a">${m} min</text>`; });
   s+=`<path d="M${X(0)} ${Y(.1)} C${X(5)} ${Y(.3)} ${X(8)} ${Y(1)} ${X(11)} ${Y(1)} S${X(20)} ${Y(.25)} ${X(30)} ${Y(.1)}" fill="#dbe8f7" stroke="#1f4e8c" stroke-width="3"/>`;
   s+=`<text x="${X(11)}" y="${Y(1)-10}" font-size="12" text-anchor="middle" fill="#1f4e8c" font-weight="700">pico da vontade</text><text x="${X(22)}" y="${Y(.5)}" font-size="12" fill="#1d2521">se você não usar, ela diminui sozinha</text>`;
   s+=`<text x="275" y="186" font-size="12" text-anchor="middle" fill="#55605a">a maioria das fissuras dura de 15 a 30 minutos</text></svg>`; return s; },
 'escada-exposicao': ()=>{ let s=`<svg viewBox="0 0 520 190" role="img" aria-label="Escada de exposição">`; const st=['Menos difícil','','','','Mais difícil'];
   for(let i=0;i<5;i++){ const x=40+i*92, y=150-i*26; s+=`<rect x="${x}" y="${y}" width="92" height="${170-y}" fill="${['#e3f0ea','#cfe5dc','#b6d8ca','#9ccab7','#7fbba2'][i]}" stroke="#fff"/><text x="${x+46}" y="${y+18}" font-size="13" text-anchor="middle" font-weight="700" fill="#1d2521">${i+1}</text>`; if(st[i]) s+=`<text x="${x+46}" y="${y-6}" font-size="11" text-anchor="middle" fill="#55605a">${st[i]}</text>`; }
   s+=`<text x="260" y="186" font-size="12" text-anchor="middle" fill="#55605a">um degrau por vez; só sobe quando o anterior ficar mais fácil</text></svg>`; return s; },
 'respiracao': ()=>{ let s=`<svg viewBox="0 0 520 150" role="img" aria-label="Respiração lenta">`; const seg=[['Inspire pelo nariz','4 segundos','#dbe8f7','#1f4e8c',130],['Pausa','2 segundos','#e7ecea','#55605a',80],['Solte devagar pela boca','6 segundos','#e3f0ea','#035D51',190]]; let x=40;
   seg.forEach(([a,b,f,c,w])=>{ s+=`<rect x="${x}" y="30" width="${w}" height="60" rx="10" fill="${f}" stroke="${c}"/><text x="${x+w/2}" y="56" font-size="13" text-anchor="middle" font-weight="700" fill="${c}">${a}</text><text x="${x+w/2}" y="76" font-size="12" text-anchor="middle" fill="#55605a">${b}</text>`; x+=w+10; });
   s+=`<text x="260" y="125" font-size="12" text-anchor="middle" fill="#55605a">repita por 3 a 5 minutos; a expiração mais longa acalma o corpo</text></svg>`; return s; },
 'reducao-gradual': ()=>{ let s=`<svg viewBox="0 0 520 180" role="img" aria-label="Redução gradual da dose">`; const v=[1,.9,.8,.7,.6,.5,.4,.3,.2,.1,0];
   v.forEach((h,i)=>{ const x=40+i*42; s+=`<rect x="${x}" y="${150-h*110}" width="36" height="${h*110}" fill="#6b8f86"/>`; });
   s+=`<line x1="36" y1="150" x2="500" y2="150" stroke="#8a9892"/><text x="270" y="172" font-size="12" text-anchor="middle" fill="#55605a">redução em pequenos passos, com pausas; se piorar, voltar ao passo anterior</text></svg>`; return s; },
 'semaforo': ()=>{ let s=`<svg viewBox="0 0 520 90" role="img" aria-label="Sinais de alerta em três cores">`; [['Verde: estou bem','#2f8f5b'],['Amarelo: sinais de alerta','#c99a12'],['Vermelho: procurar ajuda já','#c0392b']].forEach(([t,c],i)=>{ const x=20+i*168; s+=`<circle cx="${x+20}" cy="45" r="16" fill="${c}"/><text x="${x+44}" y="50" font-size="13" font-weight="700" fill="#1d2521">${t}</text>`; }); return s+'</svg>'; },
 'rotina-dia': ()=>{ let s=`<svg viewBox="0 0 520 120" role="img" aria-label="Rotina diária regular">`; const it=[['Acordar','mesmo horário'],['Refeições','horários fixos'],['Atividade','sol e movimento'],['Remédios','sempre iguais'],['Deitar','mesmo horário']];
   it.forEach(([a,b],i)=>{ const x=20+i*100; s+=`<circle cx="${x+40}" cy="40" r="26" fill="#e3f0ea" stroke="#035D51" stroke-width="2"/><text x="${x+40}" y="45" font-size="13" text-anchor="middle" font-weight="700" fill="#035D51">${i+1}</text><text x="${x+40}" y="86" font-size="12" text-anchor="middle" font-weight="700" fill="#1d2521">${a}</text><text x="${x+40}" y="102" font-size="11" text-anchor="middle" fill="#55605a">${b}</text>`; if(i<4) s+=`<line x1="${x+68}" y1="40" x2="${x+112}" y2="40" stroke="#8a9892" stroke-width="2"/>`; }); return s+'</svg>'; }
};
function cicloSVG(et,tit){ const n=et.length, cx=260, cy=120, R=82; let s=`<svg viewBox="0 0 520 250" role="img" aria-label="${esc(tit)}"><defs><marker id="pe-ar" viewBox="0 0 10 10" refX="8" refY="5" markerWidth="7" markerHeight="7" orient="auto"><path d="M0 0L10 5L0 10z" fill="#8a9892"/></marker></defs>`;
  const pt=i=>{ const a=-Math.PI/2+i*2*Math.PI/n; return [cx+R*Math.cos(a), cy+R*Math.sin(a)]; };
  for(let i=0;i<n;i++){ const a1=-Math.PI/2+i*2*Math.PI/n+.32, a2=-Math.PI/2+(i+1)*2*Math.PI/n-.32; const r=R; s+=`<path d="M${cx+r*Math.cos(a1)} ${cy+r*Math.sin(a1)} A${r} ${r} 0 0 1 ${cx+r*Math.cos(a2)} ${cy+r*Math.sin(a2)}" fill="none" stroke="#8a9892" stroke-width="2" marker-end="url(#pe-ar)"/>`; }
  et.forEach((t,i)=>{ const [x,y]=pt(i); const w=Math.min(150,Math.max(96,t.length*6.2)); const lines=t.length>22?[t.slice(0,t.lastIndexOf(' ',22)),t.slice(t.lastIndexOf(' ',22)+1)]:[t];
    s+=`<rect x="${x-w/2}" y="${y-(lines.length>1?20:13)}" width="${w}" height="${lines.length>1?40:26}" rx="8" fill="${i===0?'#dbe8f7':'#f1f4f2'}" stroke="${i===0?'#1f4e8c':'#b9c4be'}"/>`+lines.map((l,k)=>`<text x="${x}" y="${y+(lines.length>1?(k?14:-2):4)}" font-size="11.5" text-anchor="middle" fill="#1d2521">${esc(l)}</text>`).join(''); });
  return s+`<text x="${cx}" y="244" font-size="12" text-anchor="middle" fill="#55605a">${esc(tit)}: a saída é quebrar o ciclo em algum ponto</text></svg>`; }
const peSlug = (g,id)=>COND_SLUG_INV(g)+'--'+id;
function COND_SLUG_INV(g){ return slugCond(g); }
function peBloco(b){
  const T=b.titulo?`<h3>${esc(b.titulo)}</h3>`:'';
  const L=(x,cl)=>`<ul class="${cl||''}">${arr(x).map(t=>`<li>${md(t)}</li>`).join('')}</ul>`;
  switch(b.tipo){
    case 'lista': return `<section class="pe-b">${T}${b.texto?`<p>${md(b.texto)}</p>`:''}${L(b.itens)}</section>`;
    case 'passos': return `<section class="pe-b">${T}${b.texto?`<p>${md(b.texto)}</p>`:''}<ol class="pe-passos">${arr(b.itens).map(t=>`<li>${md(t)}</li>`).join('')}</ol></section>`;
    case 'checklist': return `<section class="pe-b">${T}${b.texto?`<p>${md(b.texto)}</p>`:''}${L(b.itens,'pe-check')}</section>`;
    case 'fazer': return `<section class="pe-b pe-fz">${T}<div class="pe-2"><div class="pe-sim"><h4>Faça</h4>${L(b.sim)}</div><div class="pe-nao"><h4>Evite</h4>${L(b.nao)}</div></div></section>`;
    case 'semaforo': return `<section class="pe-b">${T}<div class="pe-sem"><div class="s-v"><h4>Verde: estou bem</h4>${L(b.verde)}</div><div class="s-a"><h4>Amarelo: atenção</h4>${L(b.amarelo)}</div><div class="s-r"><h4>Vermelho: procurar ajuda</h4>${L(b.vermelho)}</div></div></section>`;
    case 'mitos': return `<section class="pe-b">${T}<div class="pe-mitos">${arr(b.itens).map(m=>`<div><p class="pe-mito"><b>Mito:</b> ${md(m.mito)}</p><p class="pe-fato"><b>Fato:</b> ${md(m.fato)}</p></div>`).join('')}</div></section>`;
    case 'tabela': return `<section class="pe-b">${T}${b.texto?`<p>${md(b.texto)}</p>`:''}<table class="pe-t"><thead><tr>${arr(b.colunas).map(c=>`<th>${esc(c)}</th>`).join('')}</tr></thead><tbody>${arr(b.linhas).map(r=>`<tr>${r.map(c=>`<td>${md(c)}</td>`).join('')}</tr>`).join('')}${Array.from({length:b.linhas_vazias||0}).map(()=>`<tr>${arr(b.colunas).map(()=>'<td>&nbsp;</td>').join('')}</tr>`).join('')}</tbody></table></section>`;
    case 'destaque': return `<section class="pe-b pe-dest">${T}<p>${md(b.texto||'')}</p></section>`;
    case 'anotar': return `<section class="pe-b">${T}${b.texto?`<p>${md(b.texto)}</p>`:''}<div class="pe-linhas">${Array.from({length:b.linhas_vazias||4}).map(()=>'<div></div>').join('')}</div></section>`;
    default: return `<section class="pe-b">${T}${String(b.texto||'').split(/\n\n+/).map(t=>`<p>${md(t)}</p>`).join('')}</section>`;
  }
}
function peFolha(g,s){
  const fig=s.figura&&PE_FIG[s.figura]?`<figure class="pe-fig">${PE_FIG[s.figura]()}${s.figura_legenda?`<figcaption>${md(s.figura_legenda)}</figcaption>`:''}</figure>`:'';
  return `<article class="pe-folha">
    <header class="pe-h"><div class="pe-marca"><svg viewBox="0 0 32 32" aria-hidden="true"><rect x="0.5" y="0.5" width="31" height="31" rx="7" fill="#fff" stroke="#cfd6d2"/><path d="M8.6 11.6h7.6v8.8H8.6a4.4 4.4 0 0 1 0-8.8z" fill="#035D51"/><rect x="16.2" y="11.6" width="10.1" height="8.8" fill="#1967CD"/></svg><span>Material de orientação · ${esc(g)}</span></div><span class="pe-pub">Para: ${esc(s.publico||'paciente e família')}</span></header>
    <h1>${esc(s.titulo)}</h1>${s.subtitulo?`<p class="pe-sub">${md(s.subtitulo)}</p>`:''}
    ${s.figura_posicao==='fim'?'':fig}
    ${arr(s.blocos).map(peBloco).join('')}
    ${s.figura_posicao==='fim'?fig:''}
    ${arr(s.ajuda).length?`<section class="pe-b pe-ajuda"><h3>Procure ajuda imediatamente se</h3><ul>${s.ajuda.map(t=>`<li>${md(t)}</li>`).join('')}</ul><p class="pe-cvv">Emergência: SAMU 192 · CVV (apoio emocional, 24 horas): ligue 188 ou acesse cvv.org.br</p></section>`:''}
    <footer class="pe-rod"><div class="pe-prof"><span>Profissional:</span><span>Contato:</span><span>Próxima consulta:</span></div><p>Material educativo. Não substitui a consulta nem muda doses por conta própria: converse com sua equipe de saúde antes de qualquer mudança.${s.fonte?` Baseado em: ${esc(s.fonte)}.`:''}</p></footer>
  </article>`;
}
function imprimirPE(){
  const emFrame=(()=>{ try{ return window.self!==window.top; }catch(e){ return true; } })();
  if(emFrame){ toast('Na versão publicada, a impressão é bloqueada. Abra o arquivo offline (guia-psicofarmacos.html) para imprimir ou salvar em PDF.'); return; }
  setTimeout(()=>window.print(),200);
}
function renderPsico(arg){
  const M=document.getElementById('main'); const [slug,id]=String(arg||'').split('--'); const g=COND_SLUG[slug];
  const s=g&&arr(PE[g]).find(x=>x.id===id);
  if(!s){ renderPsicoLista(); return; }
  const outras=arr(PE[g]).filter(x=>x!==s);
  M.innerHTML=`<div class="pe-barra noprint"><a href="#condicao-${slug}">← ${esc(g)}</a><span class="pe-acoes"><button class="btn" id="pe-print">Imprimir ou salvar PDF</button></span></div>
    <div class="pe-papel">${peFolha(g,s)}</div>
    ${outras.length?`<div class="noprint pe-outras"><h2 class="itx-h">Outras fichas desta condição</h2><div class="pe-cards">${outras.map(x=>peCard(g,x)).join('')}</div></div>`:''}`;
  document.getElementById('pe-print').onclick=imprimirPE;
  document.title=s.titulo+' · Psicoeducação';
}
function peCard(g,s){ return `<a class="pe-card" href="#psico-${slugCond(g)}--${s.id}"><span class="pe-card-p">${esc(s.publico||'Paciente e família')}</span><b>${esc(s.titulo)}</b>${s.subtitulo?`<span>${esc(s.subtitulo)}</span>`:''}</a>`; }
function peSecao(g){ const l=arr(PE[g]); if(!l.length) return ''; return `<div class="pe-cards">${l.map(s=>peCard(g,s)).join('')}</div><p class="small muted">Fichas em linguagem simples para entregar ao paciente e à família. Abra a ficha e use “Imprimir ou salvar PDF” (no arquivo offline).</p>`; }
function renderPsicoLista(){
  const M=document.getElementById('main'); const gs=Object.keys(PE).filter(g=>arr(PE[g]).length&&COND[g]!==undefined||arr(PE[g]).length).sort((a,b)=>a.localeCompare(b,'pt'));
  M.innerHTML=`<div class="page-h"><h1>Psicoeducação</h1><p>${PE_N} fichas para pacientes e famílias, organizadas por condição, prontas para imprimir. Linguagem simples, figuras e espaço para anotações da equipe.</p></div>
  ${gs.map(g=>`<section class="cbloco"><h2 class="itx-h"><a href="#condicao-${slugCond(g)}">${esc(g)}</a></h2><div class="pe-cards">${arr(PE[g]).map(s=>peCard(g,s)).join('')}</div></section>`).join('')}`;
  document.title='Psicoeducação · Guia de Psicofármacos';
}

/* ---------- CAMADA DO GUIA DE PRESCRIÇÃO (Stahl, 6.ª ed., 2017) ---------- */
const ST_TAG='<span class="st-tag" title="Conteúdo baseado no Stahl’s Prescriber’s Guide, 6.ª edição (2017)">Stahl</span>';
const stH=t=>`<h3 class="st-h">${t} ${ST_TAG}</h3>`;
const stL=l=>arr(l).length?lista(l):'';
function stAlvos(f){ const s=f.stahl; if(!s||!arr(s.alvos).length) return ''; return `${stH('Sintomas-alvo principais')}<div class="tags st-alvos">${s.alvos.map(a=>`<span class="chip">${esc(a)}</span>`).join('')}</div>`; }
function stPresc(f){ const s=f.stahl; if(!s) return ''; let h='';
  if(s.latencia||arr(s.se_funciona).length||arr(s.se_nao_funciona).length) h+=stH('Resposta ao tratamento')+kv([['Tempo até o efeito',s.latencia],['Se funcionar',s.se_funciona],['Se não funcionar',s.se_nao_funciona],['Melhores associações',arr(s.potencializacao).join(' · ')]]);
  if(arr(s.dicas_dose).length) h+=stH('Dicas de dose')+stL(s.dicas_dose);
  if(arr(s.exames).length) h+=stH('Exames')+stL(s.exames);
  if(s.tabela){ const t=s.tabela; h+=stH(esc(t.titulo))+`<div class="scroll-x"><table class="itable"><thead><tr>${t.colunas.map(c=>`<th>${esc(c)}</th>`).join('')}</tr></thead><tbody>${t.linhas.map(r=>`<tr>${r.map((c,i)=>`<td>${i===0?'<b>'+esc(c)+'</b>':md(c)}</td>`).join('')}</tr>`).join('')}</tbody></table></div>${t.nota?`<p class="small muted">${md(t.nota)}</p>`:''}`; }
  return h; }
function stAdv(f){ const s=f.stahl; if(!s) return ''; let h='';
  if(arr(s.mecanismo_ea).length) h+=stH('Por que causa efeitos adversos')+stL(s.mecanismo_ea);
  if(s.peso||s.sedacao) h+=kv([['Ganho de peso',s.peso],['Sedação',s.sedacao]]);
  if(arr(s.ea_graves).length) h+=`<div class="callout">${stH('Potencialmente graves')}${stL(s.ea_graves)}</div>`;
  if(arr(s.manejo).length) h+=stH('Manejo detalhado')+stL(s.manejo);
  return h; }
function stInter(f){ const s=f.stahl; if(!s||!arr(s.interacoes).length) return ''; return stH('Interações destacadas')+stL(s.interacoes); }
function stPop(f){ const s=f.stahl; if(!s) return ''; let h='';
  if(arr(s.nao_usar).length) h+=`<div class="callout">${stH('Não usar')}${stL(s.nao_usar)}</div>`;
  if(arr(s.advertencias).length) h+=stH('Outras advertências')+stL(s.advertencias);
  const P=s.pop||{}; const lab=[['renal','Insuficiência renal'],['hepatica','Insuficiência hepática'],['cardiaca','Cardiopatia'],['idosos','Idosos'],['criancas','Crianças e adolescentes'],['gestacao','Gestação'],['lactacao','Lactação']];
  const pp=lab.filter(([k])=>P[k]); if(pp.length) h+=stH('Populações especiais: detalhes')+kv(pp.map(([k,l])=>[l,P[k]]));
  return h; }
function stSusp(f){ const s=f.stahl; if(!s) return ''; let h='';
  if(s.longo_prazo||s.dependencia||arr(s.suspender).length) h+=stH('Uso prolongado e retirada')+kv([['Uso prolongado',s.longo_prazo],['Potencial de dependência',s.dependencia],['Como suspender',s.suspender]]);
  if(arr(s.troca).length) h+=stH('Como trocar')+stL(s.troca);
  return h; }
function stOver(f){ const s=f.stahl; if(!s||!s.superdosagem) return ''; return kv([['Segundo o Stahl',s.superdosagem]]); }
function stArte(f){ const s=f.stahl; if(!s) return ''; let h='';
  if(arr(s.vantagens).length||arr(s.desvantagens).length) h+=`${stH('Vantagens e desvantagens')}<div class="cols2"><div class="rc-b sim"><h3>Vantagens potenciais</h3>${stL(s.vantagens)}</div><div class="rc-b nao"><h3>Desvantagens potenciais</h3>${stL(s.desvantagens)}</div></div>`;
  if(arr(s.perolas).length) h+=stH('Pérolas do guia de prescrição')+s.perolas.map(p=>`<div class="perola">${md(p)}</div>`).join('');
  return h; }

/* rolagem até uma seção, descontando cabeçalho e índice fixos */
let rolando=0;
function irPara(el, barra){
  if(!el) return;
  const top=document.querySelector('.top'); let off=(top?top.getBoundingClientRect().height:0)+12;
  if(barra){ const cs=getComputedStyle(barra); if(cs.position==='sticky' && cs.flexDirection!=='column') off+=barra.getBoundingClientRect().height; }
  const y=Math.max(0, el.getBoundingClientRect().top + window.scrollY - off);
  rolando=Date.now();
  try{ window.scrollTo({top:y, behavior:'smooth'}); }catch(e){ window.scrollTo(0,y); }
  setTimeout(()=>{ if(Math.abs(window.scrollY-y)>40) window.scrollTo(0,y); }, 900);
}
function centraChip(bar,a){ if(!bar||!a) return; const l=a.offsetLeft-bar.clientWidth/2+a.clientWidth/2; bar.scrollTo?bar.scrollTo({left:Math.max(0,l)}):(bar.scrollLeft=l); }
/* ---------- FICHA ---------- */
...
function renderFicha(f){
  ...
  M.innerHTML = `<div class="ficha">
   ...
   <article>
    ...
    ${sec('interacoes',4,'Interações medicamentosas',
      ...
      autoInter(f)+`<p class="small muted" style="margin-top:10px">Para combinar três ou mais fármacos, use o <a href="#interacoes">verificador de interações</a>.</p>`+stInter(f))}
    ${sec('indicacoes',5,'Indicações e usos clínicos',stAlvos(f)+usosFicha(f))}
    ${sec('prescricao',6,'Como prescrever',
      notaClasse(f,'prescricao')+kv([['Dose inicial',pr.dose_inicial],['Faixa habitual',pr.faixa],['Dose máxima',pr.dose_maxima],['Titulação',pr.titulacao],['Horário',pr.horario],['Tempo até o efeito',pr.latencia],['Duração',pr.duracao]])+titulacaoSVG(pr.esquema)+(arr(pr.monitoramento).length?'<h3>Monitoramento</h3>'+lista(pr.monitoramento):'')+stPresc(f))}
    ${sec('adversos',7,'Efeitos adversos',
      ...
      (arr(ea.manejo).length?`<h3>Manejo dos mais comuns</h3><div class="kv">${arr(ea.manejo).map(m=>`<div>${esc(m.efeito)}</div><div>${md(m.conduta)}</div>`).join('')}</div>`:'')+stAdv(f))}
    ${sec('populacoes',8,'Contraindicações, precauções e populações especiais',
      ...
      `<div class="pop" style="margin-top:12px">${POPS.map(([k,l])=>{ const p=po[k]; if(!p) return ''; const o=typeof p==='string'?{texto:p}:p; return `<div class="popc"><h4>${l}${o.nivel?`<span class="nv ${esc(o.nivel)}">${esc(o.nivel==='aceitavel'?'aceitável':o.nivel)}</span>`:''}</h4><p>${md(o.texto)}</p></div>`; }).join('')}</div>`+stPop(f))}
    ${sec('suspensao',9,'Suspensão e troca',
      ...
      `<p class="small" style="margin-top:8px"><a href="#trocas">Abrir o guia de trocas</a></p>`+stSusp(f))}
    ${sec('superdosagem',10,'Superdosagem',notaClasse(f,'superdosagem')+kv([['Toxicidade',od.toxicidade],['Quadro clínico',od.quadro],['Conduta',od.conduta]])+stOver(f))}
    ${sec('pratica',11,'Aspectos práticos e peculiaridades',
      ...
      (arr(pt.erros).length?'<h3>Erros comuns de prescrição</h3>'+lista(pt.erros):'')+stArte(f)+comparacoes(f)+
  ...
  toc.querySelectorAll('a').forEach(a=>a.addEventListener('click',e=>{ e.preventDefault(); irPara(document.getElementById('s-'+a.dataset.s), toc); }));
  const secs=[...document.querySelectorAll('.fsec')];
  const onScroll=()=>{ let cur=secs[0].id; secs.forEach(s=>{ if(s.getBoundingClientRect().top<160) cur=s.id; }); toc.querySelectorAll('a').forEach(a=>{ const on='s-'+a.dataset.s===cur; a.classList.toggle('on',on); if(on&&window.innerWidth<900&&Date.now()-rolando>1000) centraChip(toc,a); }); };
...
function planoTroca(o,d,grupo){
  ...
  if(grupo==='antidepressivos'){
    ...
    // antipsicóticos orais: grupos de retirada do Stahl (The Art of Switching)
    const RA=R.retirada_ap||{}; const grp=f=>Object.keys(RA).find(k=>arr(RA[k].ids).includes(f.id));
    const gO=grp(o)||'uma_semana', G=RA[gO]||{semanas:1,texto:''};
    const sed=arr(R.sedativos_ap).includes(d.id);
    const parcial=pd.agonista_parcial && !po.agonista_parcial;
    const dur=G.semanas;
    // introdução do destino
    let pIni, dest;
    if(d.id==='clozapina'){ pIni=`Titular a clozapina desde 12,5 mg à noite, com aumentos de 25 a 50 mg a cada 2 a 3 dias conforme a tolerância${gO==='uma_semana'?' (Stahl: vindo de risperidona, ziprasidona ou lurasidona, a titulação deve levar pelo menos 2 semanas, para tolerância à sedação)':''}.`; dest=[[0,0],[1,.3],[2,.6],[3,.85],[4,1]]; }
    else if(sed && gO==='imediata'){ pIni=`Suspender ${lc(nO)} e iniciar ${lc(nD)} já em dose intermediária (Stahl).`; dest=[[0,0],[.05,.5],[1,1]]; }
    else if(sed){ pIni=`Iniciar ${lc(nD)} de forma gradual, titulando em pelo menos 2 semanas para permitir tolerância à sedação (Stahl).`; dest=[[0,0],[1,.5],[2,1]]; }
    else { pIni=`Iniciar ${lc(nD)} em dose intermediária e chegar à dose-alvo em 3 a 7 dias (Stahl).`; dest=[[0,0],[.05,.5],[1,1]]; }
    passos.push(pIni);
    // retirada da origem
    let orig;
    const hold = (parcial && gO!=='imediata') ? (d.id==='cariprazina'?2.5:1) : (d.id==='clozapina'&&o.id!=='clozapina'? (gO==='imediata'?0:2) : 0);
    if(gO==='imediata' && d.id!=='clozapina'){
      passos.push(`${nO} ${G.texto}.${parcial?' Se houver receio de piora, pode-se manter por alguns dias enquanto '+lc(nD)+' sobe.':''}`);
      orig=[[0,1],[.05,0]];
    } else if(gO==='imediata'){
      passos.push(`${nO} ${G.texto}; como a clozapina precisa de titulação lenta, manter ${lc(nO)} até a clozapina chegar a doses eficazes e então suspender.`);
      orig=[[0,1],[3,1],[3.1,0]];
    } else {
      if(hold) passos.push(`Manter ${lc(nO)} na dose plena ${d.id==='cariprazina'?'por 2 a 3 semanas (a cariprazina demora semanas para o equilíbrio)':d.id==='clozapina'?'até a clozapina estar em 200 a 300 mg/dia':'por alguns dias a 3 a 4 semanas, até '+lc(nD)+' estar em dose plena (troca em platô)'}.`);
      passos.push(`Depois, ${lc(nO)}: ${G.texto}.`);
      orig=[[0,1],[hold,1],[hold+dur,0]];
    }
    passos.push(`Benzodiazepínico ou anticolinérgico podem aliviar insônia, agitação ou psicose durante a transição e ser retirados um a um depois${d.id==='clozapina'?'; com clozapina, benzodiazepínico exige cautela (colapso circulatório)':''}.`);
    if(gO==='convencional') passos.push(`Se houver antiparkinsoniano associado, mantê-lo por algumas semanas após suspender ${lc(nO)} (Stahl).`);
    tipo = d.id==='clozapina'?'Titulação da clozapina com retirada posterior' : o.id==='clozapina'?'Retirada muito lenta' : gO==='imediata'?'Troca direta' : hold?'Troca em platô (plateau cross-titration)' : dur>=3?'Troca cruzada com retirada lenta':'Troca cruzada curta';
    if(o.id==='clozapina'){ alertas.push('Suspensão abrupta da clozapina associa-se a psicose de rebote grave e a síndrome colinérgica. Se inevitável (neutropenia, miocardite), cobrir o rebote colinérgico com anticolinérgico.'); }
    if(parcial) alertas.push('Retirar o antagonista pleno antes de o agonista parcial estar em dose plena favorece piora psicótica, acatisia e insônia (rebote de supersensibilidade D2).');
    ...
    plano={origem:orig, destino:dest, unidade:'semanas'};
  }
...
/* troca envolvendo antipsicóticos de depósito */
function planoLAI(o,d){
  const R=D.trocas, L=R.lai||{}, RA=R.retirada_ap||{};
  const iO=L[o.id], iD=L[d.id], nO=o.nome, nD=d.nome;
  if(!iO&&!iD) return planoTroca(o,d,'antipsicoticos');
  const grp=f=>Object.keys(RA).find(k=>arr(RA[k].ids).includes(f.id));
  const sed=f=>arr(R.sedativos_ap).includes(f.id);
  const esp = arr(R.especificos).find(x=>(casa(o,x.de)&&casa(d,x.para)));
  let tipo, passos=[], alertas=[], plano;
  const nomeOral=id=>id&&byId[id]?byId[id].nome:'';
  const indisp=f=>f.comercializacao&&f.comercializacao!=='disponivel';
  const nivelDep=(i)=> i.ataque? [[0,0],[.5,.7],[1.5,1]] : i.cobertura_sem>=3? [[0,0],[i.cobertura_sem-.5,.1],[i.cobertura_sem+1,.85],[i.cobertura_sem+3,1]] : i.curta? [[0,0],[.1,1],[.5,.6],[1,.2]] : [[0,0],[1,.5],[i.cobertura_sem+1,.85],[i.cobertura_sem+3,1]];
  if(!iO && iD){
    // oral -> depósito
    const mesma = iD.oral===o.id;
    tipo = iD.curta?'Oral para acetato (uso agudo)':'Oral para depósito';
    if(!mesma && iD.oral && !iD.curta) passos.push(`Estabelecer antes a tolerância a ${lc(nomeOral(iD.oral))}: trocar ${lc(nO)} pela forma oral (ver a aba Antipsicóticos) ou, no mínimo, testar algumas doses orais antes da primeira aplicação (Stahl).`);
    passos.push(`**Início de ${nD}:** ${iD.inicio}`);
    passos.push(`**Cobertura:** ${iD.cobertura}`);
    const G=RA[grp(o)]||RA.uma_semana||{semanas:1,texto:'reduzir em cerca de 1 semana'};
    if(iD.curta) passos.push(`Quando o quadro agudo ceder, seguir para ${lc(nomeOral(iD.oral))} oral ou decanoato (aba Depósito: acetato → decanoato).`);
    else if(mesma) passos.push(`Ao fim da cobertura, suspender ${lc(nO)}: é a mesma molécula do depósito.`);
    else passos.push(`Depois da cobertura, ${lc(nO)}: ${G.texto}.`);
    passos.push('Benzodiazepínico ou anticolinérgico em uso podem ser mantidos durante a transição e retirados um a um quando o paciente estiver estável no depósito (Stahl).');
    const c=iD.cobertura_sem||0, dur=mesma?0.05:(G.semanas||0.05);
    plano={origem: iD.curta?[[0,1],[.05,.3],[1,.3]]:[[0,1],[c,1],[c+Math.max(dur,.05),0]], destino:nivelDep(iD)};
    if(!iD.ataque && !iD.curta) alertas.push('Sem ataque ou cobertura oral adequados, os níveis do depósito ficam subterapêuticos por semanas a meses (cinética de flip-flop: a absorção lenta do local da aplicação dita o tempo até o equilíbrio).');
    if(iD.agonista_parcial && !(o.troca||{}).agonista_parcial) alertas.push('Vindo de antagonista D2 pleno, retirar o oral anterior devagar reduz piora psicótica, acatisia e insônia de rebote.');
  } else if(iO && !iD){
    // depósito -> oral
    tipo='Depósito para oral';
    const mesma = iO.oral===d.id;
    passos.push(`**Quando iniciar:** ${iO.para_oral}`);
    passos.push(mesma?`Retomar ${lc(nD)} na dose oral equivalente à do depósito, ajustando pela resposta.`: sed(d)?`Titular ${lc(nD)} em 1 a 2 semanas, somando ao depósito que ainda decai: vigiar sedação e hipotensão.`:`Iniciar ${lc(nD)} em dose baixa a intermediária e ajustar em 1 a 2 semanas conforme a resposta.`);
    passos.push('Vigiar recaída nos 2 a 3 meses seguintes: o depósito sai devagar, e a recaída pode vir tardia, quando a adesão ao oral já falhou.');
    if(d.id==='clozapina') passos.push('Clozapina: titulação habitual desde 12,5 mg, com hemograma e vigilância de miocardite; a soma com o depósito que decai aumenta sedação e hipotensão.');
    const dec = o.id==='risperidona-lp'?[[0,1],[2,1],[4,.4],[6,0]]:[[0,1],[2,.6],[4,.35],[7,.12],[10,0]];
    plano={origem:dec, destino: iO.curta?[[0,0],[.4,0],[1,.6],[2,1]]:(sed(d)?[[0,0],[1,.5],[2,1]]:[[0,0],[.05,.4],[1.5,1]])};
    if(o.id==='paliperidona-palmitato') alertas.push('No palmitato trimestral, os níveis persistem por meses: o momento de início do oral é a data em que venceria a próxima aplicação trimestral.');
  } else {
    // depósito -> depósito
    tipo='Depósito para depósito';
    if(o.id==='zuclopentixol-acetato' && iD.oral==='zuclopentixol'){ passos.push(`**Transição:** ${iO.para_decanoato}`); passos.push(`**Esquema de ${nD}:** ${iD.inicio}`); plano={origem:[[0,1],[.5,.5],[1,0]], destino:[[0,0],[.5,.4],[1.5,.8],[3,1]]}; }
    else {
      if(iD.curta){ passos.push('O acetato não substitui um depósito de manutenção: serve para agitação aguda por até 2 semanas.'); }
      if(iO.curta) passos.push(`Aplicar ${lc(nD)} 2 a 3 dias após a última aplicação do acetato, com o esquema de início e a cobertura que ${lc(nD)} exige.`);
      else passos.push(`Fazer a última aplicação de ${lc(nO)} normalmente. **${nD}:** ${iD.de_lai}`);
      if(iD.oral && iO.oral!==iD.oral && !iD.curta) passos.push(`Se o paciente nunca usou ${lc(nomeOral(iD.oral))}, testar antes a tolerância oral (Stahl).`);
      passos.push(`**Esquema de ${nD}:** ${iD.inicio_de_lai||iD.inicio}`);
      if(iD.cobertura_sem && !iD.curta) passos.push(`**Cobertura:** ${iD.cobertura}`);
      plano={origem:[[0,1],[2,.6],[4,.35],[7,.12],[10,0]], destino: iD.cobertura_sem>=3?[[0,0],[2.5,.1],[4,.8],[6,1]]:iD.curta?[[0,0],[.1,1],[.6,.5],[1,.2]]:[[0,0],[1,.5],[3,.85],[6,1]]};
    }
    alertas.push('Durante semanas os dois depósitos se somam no organismo: vigiar EPS, sedação e prolactina nesse período, e ajustar a dose do novo pela resposta, não só pela conversão.');
  }
  [o,d].forEach(f=>{ if(indisp(f)) alertas.push(`${f.nome}: não comercializado no Brasil.`); });
  const iNv=(iD&&iD.nivel)||''; if(iNv) alertas.push(`Nível sérico de ${nD}: ${iNv}`);
  const total=Math.ceil(Math.max(...plano.origem.map(p=>p[0]),...plano.destino.map(p=>p[0]))+.5);
  return {tipo,passos,alertas,esp,lai:true,plano:{...plano,total,nomeO:nO,nomeD:nD,unidade:'semanas',legenda:'Para o injetável, a curva representa o nível aproximado do depósito em relação ao equilíbrio; para o oral, a proporção da dose-alvo. Esquema ilustrativo: o tempo 0 é a primeira aplicação (oral para depósito) ou a data em que venceria a próxima aplicação (depósito para oral ou para outro depósito).'}};
}
function trOpts(L,k){ const LA=D.trocas.lai||{}; const op=f=>`<option value="${f.id}" ${f.id===k?'selected':''}>${esc(f.nome)}</option>`; if(!L.some(f=>LA[f.id])) return L.map(op).join(''); return `<optgroup label="Orais">${L.filter(f=>!LA[f.id]).map(op).join('')}</optgroup><optgroup label="Depósito e ação prolongada">${L.filter(f=>LA[f.id]).map(op).join('')}</optgroup>`; }
function renderTrocas(){
  const M=document.getElementById('main');
  const grupos={antidepressivos:F.filter(f=>f.classe==='antidepressivos'&&f.troca), antipsicoticos:F.filter(f=>f.classe==='antipsicoticos'&&f.troca&&!arr(f.marcadores).includes('lai')), deposito:F.filter(f=>f.classe==='antipsicoticos'&&(f.troca||(D.trocas.lai||{})[f.id])).sort((a,b)=>(!!(D.trocas.lai||{})[a.id])-(!!(D.trocas.lai||{})[b.id])||a.nome.localeCompare(b.nome,'pt'))};
  if(!grupos[trTab]||!grupos[trTab].length) trTab = grupos.antidepressivos.length?'antidepressivos':'antipsicoticos';
  const L=grupos[trTab]; const k1=store.get('tr_'+trTab+'_o',trTab==='deposito'?'haloperidol':L[0]&&L[0].id), k2=store.get('tr_'+trTab+'_d',trTab==='deposito'?'haloperidol-decanoato':L[1]&&L[1].id);
  M.innerHTML = `<div class="page-h"><h1>Guia de trocas</h1><p>Escolha o fármaco de origem e o de destino. O esquema considera washout de IMAO, meia-vida longa da fluoxetina, os grupos de retirada de antipsicóticos do Stahl (imediata, 1 semana, 3 a 4 semanas, clozapina), agonistas parciais e, na aba de depósito, ataque, cobertura oral e transição entre injetáveis.</p></div>
    <div class="tabs" role="tablist">${[['antidepressivos','Antidepressivos'],['antipsicoticos','Antipsicóticos'],['deposito','Antipsicóticos de depósito']].map(([k,l])=>`<button role="tab" aria-selected="${k===trTab}" data-t="${k}" ${grupos[k].length?'':'disabled'}>${l}</button>`).join('')}</div>
    ${L.length<2?'<div class="panel empty">Fichas desta classe ainda não cadastradas.</div>':`<div class="swap"><label>Origem<select id="tr-o">${trOpts(L,k1)}</select></label><label>Destino<select id="tr-d">${trOpts(L,k2)}</select></label></div><div id="tr-out"></div>`}
  ...
  const draw=()=>{ const o=byId[document.getElementById('tr-o').value], d=byId[document.getElementById('tr-d').value]; store.set('tr_'+trTab+'_o',o.id); store.set('tr_'+trTab+'_d',d.id);
    ...
    const p=trTab==='deposito'?planoLAI(o,d):planoTroca(o,d,trTab);
