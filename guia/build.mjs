...
// ajustes de usos revisados contra as diretrizes (dados/usos/ajustes/*.json): lista de { f, g, c?, set?, remove?, add?, motivo }
const ajDir = path.join(usosDir, 'ajustes');
if (fs.existsSync(ajDir)) for (const arq of fs.readdirSync(ajDir).filter(a => a.endsWith('.json')).sort()) {
  let d; try { d = rd('dados/usos/ajustes/' + arq); } catch (e) { erros.push(`JSON inválido em dados/usos/ajustes/${arq}: ${e.message}`); continue; }
  for (const [k, a] of (Array.isArray(d) ? d : []).entries()) { const w = m => erros.push(`ajustes/${arq}[${k}] ${a.f || ''}: ${m}`);
    if (!byIdF[a.f]) { w('fármaco inexistente'); continue; }
    const l = usos[a.f] = usos[a.f] || [];
    if (a.add) { const u = a.add; if (!PAPEL.includes(u.p) || !EVID.includes(u.e) || !['a', 'o'].includes(u.s) || !u.g || !u.c) { w('add inválido'); continue; }
      l.push({ condicao: u.c, grupo: u.g, status: u.s === 'a' ? 'anvisa' : 'off_label', evidencia: u.e, papel: u.p, dose: u.d, discussao: u.t, tambem: u.tambem }); continue; }
    const alvo = l.filter(u => u.grupo === a.g && (!a.c || u.condicao === a.c));
    if (!alvo.length) { avisos.push(`ajustes/${arq}[${k}] ${a.f}: uso em "${a.g}"${a.c ? ' / ' + a.c : ''} não encontrado`); continue; }
    if (a.remove) { usos[a.f] = l.filter(u => !alvo.includes(u)); continue; }
    const S = a.set || {};
    if (S.p && !PAPEL.includes(S.p)) { w('papel inválido'); continue; } if (S.e && !EVID.includes(S.e)) { w('evidência inválida'); continue; }
    alvo.forEach(u => { if (S.p) u.papel = S.p; if (S.e) u.evidencia = S.e; if (S.s) u.status = S.s === 'a' ? 'anvisa' : 'off_label'; if (S.d !== undefined) u.dose = S.d; if (S.t) u.discussao = S.t; if (S.c) u.condicao = S.c; });
  }
}
...
// camada do guia de prescrição (dados/stahl): Stahl's Prescriber's Guide, 6.ª ed. (2017)
const stDir = path.join(R, 'dados/stahl');
if (fs.existsSync(stDir)) for (const arq of fs.readdirSync(stDir).filter(a => a.endsWith('.json')).sort()) {
  let d; try { d = rd('dados/stahl/' + arq); } catch (e) { erros.push(`JSON inválido em dados/stahl/${arq}: ${e.message}`); continue; }
  for (const [id, v] of Object.entries(d)) { if (id.startsWith('_')) continue; const f = byIdF[id]; if (!f) { erros.push(`stahl/${arq}: id "${id}" não existe`); continue; }
    f.stahl = v; const fonte = 'Stahl SM. Prescriber’s Guide: Stahl’s Essential Psychopharmacology, 6.ª ed. Cambridge University Press, 2017';
    f.fontes = [...(f.fontes || []).filter(x => !/Prescriber/.test(x)), fonte]; }
}
...
// psicoeducação (dados/psicoeducacao/*.json): { "Condição": [fichas] }
const psicoeducacao = {};
const PE_FIGS = ['janela-sono','ciclo-ansiedade','ciclo-toc','ciclo-compulsao','ciclo-panico','humor-bipolar','curva-remedio','onda-fissura','escada-exposicao','respiracao','reducao-gradual','semaforo','rotina-dia'];
const PE_TIPOS = ['texto','lista','passos','checklist','fazer','semaforo','mitos','tabela','destaque','anotar'];
const peDir = path.join(R, 'dados/psicoeducacao');
if (fs.existsSync(peDir)) for (const arq of fs.readdirSync(peDir).filter(a => a.endsWith('.json')).sort()) {
  let d; try { d = rd('dados/psicoeducacao/' + arq); } catch (e) { erros.push(`JSON inválido em dados/psicoeducacao/${arq}: ${e.message}`); continue; }
  for (const [g, l] of Object.entries(d)) { if (g.startsWith('_')) continue;
    if (!condicoes[g]) avisos.push(`psicoeducacao/${arq}: condição "${g}" sem panorama`);
    const lista = psicoeducacao[g] = psicoeducacao[g] || [];
    for (const f of l) { const w = m => erros.push(`psicoeducacao/${arq} › ${g} › ${f.id || '?'}: ${m}`);
      if (!f.id || !/^[a-z0-9-]+$/.test(f.id)) { w('id inválido'); continue; } if (lista.some(x => x.id === f.id)) { w('id repetido'); continue; }
      if (!f.titulo) w('sem título'); if (f.figura && !PE_FIGS.includes(f.figura)) w(`figura "${f.figura}" inexistente`);
      (f.blocos || []).forEach((b, k) => { if (b.tipo && !PE_TIPOS.includes(b.tipo)) w(`bloco ${k}: tipo "${b.tipo}" inválido`); });
      lista.push(f); } } }
if (erros.length) { console.error('ERROS:\n' + erros.join('\n')); process.exit(1); }
if (avisos.length && process.argv.includes('-v')) console.warn('Avisos:\n' + avisos.join('\n'));

const dados = { meta, classes, farmacos, equivalencias, trocas, pares, clinicos, condicoes, psicoeducacao };
